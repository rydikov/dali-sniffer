...
Слой синхронизации состояния слушает декодированный DALI-трафик и публикует состояние устройств в MQTT. 
...
- `brightness` нужен для `brightness`, включая `QUERY_ACTUAL_LEVEL`, `OFF`, `DAPC` / `ArcPower` и follow-up после `GO_TO_SCENE`.
